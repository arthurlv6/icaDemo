            ...
            if (userName.Contains("temporary"))
            {
                ...
                else
                {
                    ...
                    while(UsersList.Where(d => d.UserID == userName).Any())
                    {
                        ...
                        userName = "client" + randomId.ToString() + "@temporary.com";
                    }
                    UsersList.Add(new UserInfo { ConnectionId = id, UserID = userName, UserName = userName, UserGroup = userGroup, freeflag = "0", tpflag = "0", });
                ...
                using (var ctx = new BaseOperations())
                {
                    ...
                    try
                    {
                        ...
                        if (!roles.Any(d => d.RoleId == "onlineChatAdmin"))
                        {
                            ...
                            //now add USER to UsersList
                            UsersList.Add(new UserInfo { ConnectionId = id, UserID = userInfo.Id, UserName = userName, UserGroup = userGroup, freeflag = "0", tpflag = "0", });
                            //whether it is Admin or User now both of them has userGroup and I Join this user or admin to specific group 
                            Groups.Add(Context.ConnectionId, userGroup);
                        ...
                        else
                        {
                            ...
                            UsersList.Add(new UserInfo { ConnectionId = id, AdminID = userInfo.Id, UserName = userName, UserGroup = userInfo.Id, freeflag = "1", tpflag = "1" });
                            //whether it is Admin or User now both of them has userGroup and I Join this user or admin to specific group 
